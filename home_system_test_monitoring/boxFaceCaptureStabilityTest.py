...
#备份盒子抓拍图片目录
def outBak(boxCaptureOut,boxCaptureOutBak,IPCOutIdxList,cycleNumber):
    src = boxCaptureOut
...
def collectDataToInfluxDB(boxIp,IPCOutIdx,detecRate,omissiveRate,repRate,misiRate):
    #now = datetime.datetime.now()
    #t = datetime.datetime.fromtimestamp(now.timestamp()).isoformat() #.strftime("%Y-%m-%dT%H:%M:%SZ")
    data_list = [{
        'measurement': 'boxFaceCapture',
        'tags': {'boxIP': boxIp,
                 'IPCIdx': IPCOutIdx},
        'fields': {
            'detecRate': detecRate,
...
if __name__ == "__main__":
    ...
    boxIPList = ["10.58.122.201"]
    boxCaptureOutList = ['/data/zcm_hezi/ws_client_01/ws_client/out/']
    boxCaptureOutBakList =['/data/zcm_hezi/ws_client_01/ws_client/outbak/']

    rtspAddrList = ["rtsp://10.58.122.65:8554/tongdao720p_proc1.264","rtsp://10.58.122.65:8554/tongdao720p_proc2.264","rtsp://10.58.122.65:8554/tongdao720p_proc3.264",
                    "rtsp://10.58.122.65:8554/tongdao720p_proc4.264","rtsp://10.58.122.65:8554/tongdao720p_proc5.264","rtsp://10.58.122.65:8554/tongdao720p_proc6.264",
                    "rtsp://10.58.122.65:8554/tongdao720p_proc7.264","rtsp://10.58.122.65:8554/tongdao720p_proc8.264"]
    tmpIdxList = ['tmp_0','tmp_1','tmp_2','tmp_3','tmp_4','tmp_5','tmp_6','tmp_7']

    baseImageDir = '/data/zcm_hezi/baseImage'
    initFolder(boxCaptureOutList, boxCaptureOutBakList, tmpIdxList)
    baseImageNameList = os.listdir(baseImageDir)
    # print('baseImageNameList------------------------------------------------------------------',baseImageNameList)
    RtspStreamTime = 65
    ...
    while True:
        try:
            ...
            for ip in boxIPList:
                ...
                print("开始登录%s的盒子进行添加视频流" % ip)
                Login(ws)
                ChnIdxList = queryRtspStream(ws)
                delRtspStream(ws, ChnIdxList)
                indx = 0
                for rtspAddr in rtspAddrList:
                    addRtspStream(ws, rtspAddr, indx)
                    indx += 1
                logOut(ws)
                ws.close()
                print("%s盒子添加视频流完成，正在等待盒子抓拍" % ip)
            # 添加视频流后等待盒子抓拍
            time.sleep(RtspStreamTime + 5)

            # 删除视频流，停止盒子抓拍
            for ip in boxIPList:
                websocketIP = "ws://" + ip + ":8000"
                ws = create_connection(websocketIP, timeout=60)
                print("开始登录%s的盒子进行删除视频流" % ip)
                Login(ws)
                ChnIdxList = queryRtspStream(ws)
                delRtspStream(ws, ChnIdxList)
                logOut(ws)
            ...
            print("抓拍完成，准备对抓拍结果就行统计分析")
            # 抓拍完成后进行抓拍结果分析
            for boxIp, boxCaptureOut, boxCaptureOutBak in zip(boxIPList, boxCaptureOutList, boxCaptureOutBakList):
                outBakDir = outBak(boxCaptureOut, boxCaptureOutBak, tmpIdxList, cycleNumber)
                for tmpIdx in tmpIdxList:
                    ...
                    client.write_points(
                        collectDataToInfluxDB(boxIp, tmpIdx, detecRate, omissiveRate, repRate, misiRate))
                    print("——————————————————————————%s盒子，%s视频流循环播放抓拍：%d次统计分析完成————————————————————" % (
                    boxIp, tmpIdx, cycleNumber))
            cycleNumber += 1
