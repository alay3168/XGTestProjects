...
#登录接口请求函数
def Login ():
    login_req = """{"Method":"Login","Page":"Login","Message":{"Login-Properties":{"UserName":"admin","Passwd":"ykl74F/yZstHWPvDCATZRA=="}}}"""
    while True:
...
#执行开始升级接口请求函数
def UpgradeStar(filename,tolalSize):
...
#传输升级包接口请求函数
def UpgradeTrans(num,file,filename,endNum):
    PackNum =1
...
#发送传输文件完成却接口请求函数
def UpgradeTransFinish(filename):
...
#开始传输文件执行升级
def Upgrade(filename,fileSize):
    file = open(filename, mode='rb')
    Num = fileSize // 5120000
    endNum = fileSize % 5120000
    UpgradeStar(filename, fileSize)
    UpgradeTrans(Num, file, filename, endNum)
    UpgradeTransFinish(filename)
    time.sleep(300)
...
if __name__ == "__main__":
    #如下配置信息需要按照实际情况进行配置
    deviceIP = ["10.58.122.108","10.58.122.201"]
    ...
    fileName1 = "xgai1527a.bin"#升级文件1的文件名，需要根据实际情况配置
    fileName2 = "xgai1532d.bin"#升级文件2的文件名，需要根据实际情况配置

    fileSize1 = os.path.getsize("xgai1527a.bin")#升级文件1的大小，需要根据实际情况配置
    fileSize2 = os.path.getsize("xgai1532d.bin")#升级文件2的大小，需要根据实际情况配置
    ...
    while number<=10000:
        ...
        try:
            ...
            if DevFW == Version1:
                ...
                #print(str(datetime.now())+"：第%d次升级出现异常执行getlog.sh保存日志至/usr/local/stor/logbak，:%s，"%(number,e))
            errorMessage = str(datetime.now())+"：第%d次升级出现异常执行getlog.sh保存日志至/usr/local/stor/logbak，:%s，"%(number,e)
            print(errorMessage)
            with open(autoUpdateFailLog, 'a') as f:
                f.write(errorMessage)
                f.write('\n')
