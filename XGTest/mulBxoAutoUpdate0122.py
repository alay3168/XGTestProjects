...
def UpgradeStar(boxIP,ws,filename,tolalSize,number):
    ...
    if RetCode !=0:
        errorMessage = str(datetime.now())+"：第%d次升级，%s盒子执行star接口开始传输升级包失败，其返回信息：%s，执行getlog.sh备份盒子升级日志"%(number,boxIP,starUpdateRes)
        print(errorMessage)
...
def UpgradeTrans(boxIP,ws,num,file,filename,endNum,number):
    ...
    while True:
        ...
        if num:
            ...
            if RetCode !=0:
                errorMessage = str(datetime.now()) + "：第%d次升级，%s盒子执行Trans接口传输文件失败,其返回信息：%s，执行getlog.sh备份盒子升级日志"%(number,boxIP,transUpdateReqEndRes)
                print(errorMessage)
                cmd_res(boxIP, port, username, password, finish, cmds[2])
                cmd_res(boxIP, port, username, password, finish, cmds[4])
                with open(autoUpdateFailLog, 'a') as f:
...
def UpgradeTransFinish(boxIP,ws,filename,number):
    ...
    if starUpdateRes["RetCode"] is 0:
        ...
        errorMessage = str(datetime.now()) + "：第%d次升级，%s盒子执行TransFinish接口确认传输完整升级包文件失败,其返回信息：%s，执行getlog.sh备份盒子升级日志"%(number,boxIP,starUpdateRes)
        print(errorMessage)
        cmd_res(boxIP, port, username, password, finish, cmds[2])
        cmd_res(boxIP, port, username, password, finish, cmds[4])
        with open(autoUpdateFailLog, 'a') as f:
...
def Upg_to_confirm(boxIP,BoxMainMD5,BoxMaindebugMD5,wwwMD5,number):
    ...
    if BoxMainMD5 == BMMD5 and BoxMaindebugMD5 == BMDMD5 and wwwMD5 == wMD5:
        ...
        errorMessage = str(datetime.now()) + "：——————————————————————————————第：%d次升级，%s盒子执行MD5校验失败,执行getlog.sh保存日志至/usr/local/stor/logbak——————————————————————————————" % (number,boxIP)
        print(errorMessage)
        ...
        try:
            ...
            print(str(
                datetime.now()) + "：——————————————————————————————%s盒子开始执行第：%d次升级，升级的版本是%s,升级的文件包是%s——————————————————————————————" % (
                      boxIP, number, updateFileList[j][0], updateFileList[j][1]))
            ...
            print("%s盒子执行%s版本%s文件包进行升级共耗时:%s" % (
            boxIP, updateFileList[j][0], updateFileList[j][1], str(endTime - startTime)))
            print(str(
                endTime) + "：——————————————————————————————%s盒子执行%s版本%s文件包升级MD5校验成功，第：%d次升级成功——————————————————————————————" % (
                  boxIP, updateFileList[j][0], updateFileList[j][1], number))
            number += 1
        except Exception as e:
            errorMessage = str(datetime.now()) + "：%s盒子第%d次执行%s版本%s文件包升级出现异常执行getlog.sh保存日志至/usr/local/stor/logbak，:%s，" % (boxIP,number,updateFileList[j][0], updateFileList[j][1], e)
            print(errorMessage)
            with open(autoUpdateFailLog, 'a') as f:
                f.write('\n')
                f.write(errorMessage)
...
if __name__ == "__main__":
    ...
    updateFileList = [["v1.0.0.20200117152700","xgaia1.bin",os.path.getsize("xgaia1.bin"),"9de9117487c27fa4f395d33e929e9d2f","6e607944ee2a46a6e4fa73fd7ca1f83b","f081f94a3dcca313baaaada47607f6cf"],["v1.0.0.20200117153200","xgaia2.bin",os.path.getsize("xgaia2.bin"),"3b3fb09556a6a37b35066128889c490f","48cc1a9b3d63af88098450a9d2049763","aa8fd705de944184e53a21c0640aac7a"],["v1.0.0.20200117152700","xgaib1.bin",os.path.getsize("xgaib1.bin"),"9de9117487c27fa4f395d33e929e9d2f","6e607944ee2a46a6e4fa73fd7ca1f83b","f081f94a3dcca313baaaada47607f6cf"],["v1.0.0.20200117153200","xgaib2.bin",os.path.getsize("xgaib2.bin"),"3b3fb09556a6a37b35066128889c490f","48cc1a9b3d63af88098450a9d2049763","aa8fd705de944184e53a21c0640aac7a"],["v1.0.0.20200117152700","xgaic1.bin",os.path.getsize("xgaic1.bin"),"9de9117487c27fa4f395d33e929e9d2f","6e607944ee2a46a6e4fa73fd7ca1f83b","f081f94a3dcca313baaaada47607f6cf"],["v1.0.0.20200117153200","xgaic2.bin",os.path.getsize("xgaic2.bin"),"3b3fb09556a6a37b35066128889c490f","48cc1a9b3d63af88098450a9d2049763","aa8fd705de944184e53a21c0640aac7a"],["v1.0.0.20200117152700","xgaid1.bin",os.path.getsize("xgaid1.bin"),"9de9117487c27fa4f395d33e929e9d2f","6e607944ee2a46a6e4fa73fd7ca1f83b","f081f94a3dcca313baaaada47607f6cf"],["v1.0.0.20200117153200","xgaid2.bin",os.path.getsize("xgaid2.bin"),"3b3fb09556a6a37b35066128889c490f","48cc1a9b3d63af88098450a9d2049763","aa8fd705de944184e53a21c0640aac7a"],["v1.0.0.20200117152700","xgaie1.bin",os.path.getsize("xgaie1.bin"),"9de9117487c27fa4f395d33e929e9d2f","6e607944ee2a46a6e4fa73fd7ca1f83b","f081f94a3dcca313baaaada47607f6cf"],["v1.0.0.20200117153200","xgaie2.bin",os.path.getsize("xgaie2.bin"),"3b3fb09556a6a37b35066128889c490f","48cc1a9b3d63af88098450a9d2049763","aa8fd705de944184e53a21c0640aac7a"]]
