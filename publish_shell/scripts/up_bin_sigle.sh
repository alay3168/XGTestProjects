...
EpidemicPrevention=master
OperationManage_branch=master
CommunitySecurityWebSystem_branch=master
navigation_branch=master
ExhibitionWeb_branch=exh/dev/v0.2
FeiDan_Web_branch=master
AccessAuth_branch=master
AlarmSystem_branch=master
StaffRepository_branch=master
ImageStorage_branch=master
WebSystemImageService_branch=master
ImageRetrieve_branch=master
DeviceManager_branch=master
nvr_branch=master
RtspClient_branch=master
MapManager_branch=master
getway_branch=master
GrpcCommon_branch=master
Solace_branch=master
TheGood_branch=master
Watchmen_branch=master
AliyunVOD_branch=master
LogSync_branch=4oldWebImage
CaseField_branch=master
FeiDan_branch=master
authentication_branch=master
FtpImage_branch=master
BodyDetect_branch=master

# 设置程序源码目录
declare -A repo_dir
repo_dir=(
["EpidemicPrevention"]="${go_dir}"
["OperationManage"]="${go_dir}"
["AlarmSystem"]="${go_dir}"
...
# 定义所有homey程序
all_proj="EpidemicPrevention OperationManage AliyunVOD GrpcCommon authentication FtpImage grpc-getway AlarmSystem StaffRepository DeviceManager MapManager WebSystemImageService ImageRetrieve nvr RtspClient BodyDetect CommunitySecurityWebSystem navigation Solace TheGood"
all_bin_proj="EpidemicPrevention OperationManage authentication grpc-getway AlarmSystem FtpImage StaffRepository DeviceManager MapManager WebSystemImageService ImageRetrieve nvr RtspClient BodyDetect Solace TheGood CommunitySecurityWebSystem navigation"
...
function pull_and_clean_build(){
    ...
    if [ ! -e ${repo_dir[$1]}/${1} ];then
        ...
        cd ${repo_dir[$1]}/${1}
        if [ ${1} == "grpc-getway" ];then
            git checkout ${getway_branch} | tee -a ${log}
        else
            git checkout `eval echo '$'"${1}_branch"` | tee -a ${log}
        fi
    else
        cd ${repo_dir[$1]}/${1}
        ...
        if [ ${1} == "grpc-getway" ];then
            git reset --hard ${getway_branch} | tee -a ${log}
            git checkout ${getway_branch} | tee -a ${log}
        else
            git reset --hard `eval echo '$'"${1}_branch"` | tee -a ${log}
            git checkout `eval echo '$'"${1}_branch"` | tee -a ${log}
        fi
...
function build_go_common() {
    ...
    # cp bin to run_env
    if [ ${1} == "AliyunVOD" -o ${1} == "RtspClient" ];then
        cp -v ${go_dir}/${1}/server/server ${run_dir}/binary/${1} && green_echo "Copy ${1} binary to run_env Success!"
    elif [ ${1} == "authentication" ]; then
        cp -v ${go_dir}/${1}/${1} ${run_dir}/binary/${1} && green_echo "Copy ${1} binary to run_env Success!"
        cp -arp ${go_dir}/${1}/public ${run_dir}/binary/${1} 
        cp -v ${go_dir}/${1}/restful_* ${run_dir}/binary/${1}
    elif [ ${1} == "DeviceManager" ]; then
        cp -v ${go_dir}/${1}/app/onvif/libcamctrl.so ${run_dir}/binary/${1} && green_echo "Copy ${1} libcamctrl.so to run_env Success!"
        cp -v ${go_dir}/${1}/${1} ${run_dir}/binary/${1} && green_echo "Copy ${1} binary to run_env Success!"
    else
        cp -v ${go_dir}/${1}/${1} ${run_dir}/binary/${1} && green_echo "Copy ${1} binary to run_env Success!"
    fi
...
function build_nvr() {
    ...
    # stop service
    cd ${run_dir} && docker stop ${run_name}_${1} &> /dev/null
    if [ $? -ne 0 ]; then
        red_echo "Stop ${1} container unsuccessfully"
    else
        green_echo "Stop ${1} container successfully"
...
read -p "请输入要更新的程序名(例如:AlarmSystem OperationManage,以空格隔开):" projs
...
case ${i} in
  All_Homey)
    ...
    do
        ...
        cd ${repo_dir[$k]}
        if [ ! -d ${k} ];then
            git clone http://qujin@git.vision.puppyrobot.com/Intelligentvision/${k}.git
            cd ${repo_dir[$k]}/${k} > /dev/null
            if [ ${k} == "grpc-getway" ];then
                git checkout ${getway_branch} | tee -a ${log}
            else
                git checkout `eval echo '$'"${k}_branch"` | tee -a ${log}
            fi
        else
            cd ${repo_dir[$k]}/${k} > /dev/null
            git clean -f | tee -a ${log}
            ...
            if [ ${k} == "grpc-getway" ];then
                ...
                git reset --hard `eval echo '$'"${k}_branch"` | tee -a ${log}
                git checkout `eval echo '$'"${k}_branch"` | tee -a ${log}
            fi
            git pull --rebase | tee -a ${log}
        fi
        green_echo "Pull ${k} code success!"
    done
    # 开始编译
    yellow_echo "Start to build code！"
    for k in ${all_bin_proj}; do
        if [ ${k} == "AliyunVOD" -o ${k} == "RtspClient" ];then
            docker run -v ${go_dir}:/workspace/go/src/github.com/Intelligentvision/ \
            -v ${src_dir}/mod:/workspace/go/pkg/mod \
            -v ${xg_dir}:/workspace/xg_server \
            -v ${src_dir}/build_go_common.sh:/workspace/build_go_common.sh \
            --name $DOCKER_GO_NAME -i $BUILD_GO_ENV /bin/bash -c "/workspace/build_go_common.sh ${k}"
            docker rm $DOCKER_GO_NAME &> /dev/null && green_echo "rm build container $DOCKER_GO_NAME Success!"
        elif [ ${k} == "nvr" ]; then
            docker run -v ${go_dir}:/workspace/go/src/github.com/Intelligentvision/ \
            -v ${src_dir}/mod:/workspace/go/pkg/mod \
            ...
            docker run -v ${xg_dir}/BodyDetect:/workspace/BodyDetect \
            -v ${src_dir}/mod:/workspace/go/pkg/mod \
            -v ${src_dir}/build_body.sh:/workspace/build_body.sh \
            --runtime=nvidia \
            --name $DOCKER_BODY_NAME -i $BUILD_BODY_ENV /bin/bash -c "/workspace/build_body.sh"
            docker rm $DOCKER_BODY_NAME &> /dev/null && green_echo "rm build container $DOCKER_BODY_NAME Success!"
        elif [ ${k} == "Solace" ]; then
            docker run -v ${xg_dir}:/workspace/xg_server \
            -v ${src_dir}/mod:/workspace/go/pkg/mod \
        ...
        elif [ ${k} == "CommunitySecurityWebSystem" -o ${k} == "navigation" ]; then
            docker run -v ${src_dir}:/workspace \
            -v ${src_dir}/mod:/workspace/go/pkg/mod \
            --name $DOCKER_WEB_NAME -i $BUILD_WEB_ENV /bin/bash -c "/workspace/build_web_common.sh ${k}"
            docker rm $DOCKER_WEB_NAME &> /dev/null && green_echo "rm build container $DOCKER_WEB_NAME Success!"
        else
            docker run -v ${go_dir}:/workspace/go/src/github.com/Intelligentvision/ \
            -v ${src_dir}/mod:/workspace/go/pkg/mod \
    ...
    do
        if [ ${k} == "RtspClient" ];then
            cp -v ${go_dir}/${k}/server/server ${run_dir}/binary/${k} && green_echo "Copy ${k} binary to run_env Success!" || red_echo "Copy ${k} binary to run_env failed!"
        elif [ ${k} == "nvr" ]; then
            cp -v ${go_dir}/${k}/HIK_SDK_RPC/server ${run_dir}/binary/${k}/HIK_SDK_RPC/ && green_echo "Copy ${k} binary to run_env Success!" || red_echo "Copy ${k} binary to run_env failed!"
        elif [ ${k} == "BodyDetect" ]; then
            cp -v ${xg_dir}/BodyDetect/build/BodyDetect ${run_dir}/binary/${k}/build/ && green_echo "Copy ${k} binary to run_env Success!" || red_echo "Copy ${k} binary to run_env failed!"
        elif [ ${k} == "Solace" ]; then
            cp -v ${xg_dir}/Solace/build/xgface_server ${run_dir}/binary/xgface_server/Solace/build/bin && green_echo "Copy ${k} binary to run_env Success!" || red_echo "Copy ${k} binary to run_env failed!"
        elif [ ${k} == "TheGood" ]; then
            echo ${run_dir}/binary/index_server_{1..2}/TheGood/service/ | xargs -n 1 cp -v ${xg_dir}/TheGood/service/index_server && green_echo "Copy ${k} binary to run_env Success!" || red_echo "Copy ${k} binary to run_env failed!"
            echo ${run_dir}/binary/index_server_{1..2}/TheGood/ | xargs -n 1 cp -v ${xg_dir}/TheGood/libxgindex.so && green_echo "Copy libxgindex.so to run_env Success!"
        elif [ ${k} == "authentication" ]; then
            cp -v ${go_dir}/${k}/${k} ${run_dir}/binary/${k} && green_echo "Copy ${k} binary to run_env Success!" || red_echo "Copy ${k} binary to run_env failed!"
            cp -arp ${go_dir}/${k}/public ${run_dir}/binary/${k} 
            cp -v ${go_dir}/${k}/restful_model.conf ${run_dir}/binary/${k}
            cp -v ${go_dir}/${k}/restful_policy.csv ${run_dir}/binary/${k}
        elif [ ${k} == "DeviceManager" ]; then
            cp -v ${go_dir}/${k}/app/onvif/libcamctrl.so ${run_dir}/binary/${k} && green_echo "Copy ${k} libcamctrl.so to run_env Success!"
            cp -v ${go_dir}/${k}/${k} ${run_dir}/binary/${k} && green_echo "Copy ${k} binary to run_env Success!" || red_echo "Copy ${k} binary to run_env failed!"
        elif [ ${k} == "CommunitySecurityWebSystem" -o ${k} == "navigation" ]; then
            [ -e ${run_dir}/openresty/nginx/www/static/config.js ] && cp -v ${run_dir}/openresty/nginx/www/static/config.js ${run_dir}/openresty/nginx/config.js.bak
            if [ ${k} == "CommunitySecurityWebSystem" ]; then
                rm -rf ${run_dir}/openresty/nginx/www/community && mv ${src_dir}/${k}/dist/ ${run_dir}/openresty/nginx/www/community && green_echo "Copy ${k} to run_env Success!"
            else
                rm -rf ${run_dir}/openresty/nginx/www/static && mkdir -p ${run_dir}/openresty/nginx/www
                cp -arv ${src_dir}/${k}/dist/. ${run_dir}/openresty/nginx/www && green_echo "Copy ${k} to run_env Success!"
            fi
            [ -e ${run_dir}/openresty/nginx/config.js.bak ] && mv ${run_dir}/openresty/nginx/config.js.bak ${run_dir}/openresty/nginx/www/static/config.js
        else
            cp -v ${go_dir}/${k}/${k} ${run_dir}/binary/${k} && green_echo "Copy ${k} binary to run_env Success!" || red_echo "Copy ${k} binary to run_env failed!"
  ...
  Solace)
    ...
    # cp bin to run_env
    cp -v ${xg_dir}/Solace/build/xgface_server ${run_dir}/binary/xgface_server/Solace/build/bin && green_echo "Copy xgface_server successfully" || red_echo "Copy xgface_server unsuccessfully"
    [ ! -d ${run_dir}/binary/xgface_server/Solace/data ] && cp -r ${xg_dir}/xgface_data/data ${run_dir}/binary/xgface_server/Solace/ && green_echo "Copy xgface_data successfully"
    # start service
    cd ${run_dir} && docker-compose up -d
    # update commit.txt
    get_commit_info ${i}
  ...
  TheGood)
    ...
    # cp bin to run_env
    echo ${run_dir}/binary/index_server_{1..2}/TheGood/service/ | xargs -n 1 cp -v ${xg_dir}/TheGood/service/index_server && green_echo "Copy index_server successfully" || red_echo "Copy index_server unsuccessfully"
    echo ${run_dir}/binary/index_server_{1..2}/TheGood/ | xargs -n 1 cp -v ${xg_dir}/TheGood/libxgindex.so && green_echo "Copy libxgindex.so successfully" || red_echo "Copy libxgindex.so unsuccessfully"
    # start service
    cd ${run_dir} && docker-compose up -d
    # update commit.txt
    get_commit_info ${i}
  ...
  BodyDetect)
    ...
    cp -v ${xg_dir}/BodyDetect/build/BodyDetect ${run_dir}/binary/BodyDetect/build
    # start service
    cd ${run_dir} && docker-compose up -d
    # update commit.txt
    get_commit_info ${i}       
